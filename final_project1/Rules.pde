class Rules{
  ...
  Rules(float circle_x, float circle_y, float r){
  ...
  void display(){
   ...
   textSize(25);
   fill(0);
   text("Points:"+circle_num,630,100);
   popMatrix();
  ...
  boolean checkSquare(float radius_tri){
    ...
    for(Squares i: squarelist){
      if(((pointer.X2 < i.x1+radius && pointer.X2 > i.x1) || (pointer.X3 > i.x1 && pointer.X3 < i.x1+radius)) && ((pointer.Y1 < i.y1+radius && pointer.Y1 > i.y1) || (pointer.Y2 < i.y1+radius && pointer.Y2 > i.y1))){
        invincible_frame = frameCount;
        heart_num -= 1;
        return true;
    }
